...
conf_dirs = ['sssp.config.conf_clf_with_tag',]
...
def validate(valid_dset, model, sess, args, vocab, class_map):
    ...
    for batch in threaded_it:
        ...
        tgt = [class_map[idx] for idx in batch[3]]
...
def main():
    ...
    get_prepare_func = model.get_prepare_func(args)
    train_dset = initDataset(args.train_path, get_prepare_func, args.batch_size)
    valid_dset = initDataset(args.valid_path, get_prepare_func, args.batch_size)
    test_dset = initDataset(args.test_path, get_prepare_func, args.batch_size)
