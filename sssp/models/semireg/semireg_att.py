...
class SemiRegression(ModelBase):
    def __init__(self, args):
        super(SemiRegression, self).__init__()
    ...
    def _create_rnn_classifier(self, inp, msk, scope_name, args):
        ...
        with tf.variable_scope(scope_name):
            ...
            logits = tf.contrib.layers.fully_connected(enc_state, 2, scope='fc')
        return logits
    ...
    def get_loss_l(self, args):
        with tf.variable_scope(args.log_prefix):
            ...
            self.recons_loss_l, self.kl_loss_l = self._get_elbo_label(self.inp_l_plh,
                    self.tgt_l_plh,
                    self.msk_l_plh,
                    tf.concat([(1 - self.y_plh)[:,None], self.y_plh[:, None]], axis=1),
                    args)
            ...
            self.logits_l = self._create_rnn_classifier(self.tgt_l_plh,
                    ...
                    scope_name='clf',
                    args=args)
            self.predict_l = tf.nn.softmax(self.logits_l)
            self.predict_loss_l = tf.squared_difference(self.predict_l[:, 1], self.y_plh)
